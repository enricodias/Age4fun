...
class WsSys {
	...
	function parse_request () {
		if ($_GET)    {foreach ($_GET    as $k => $v) {$this->req[$k] = $this->fetch_request($v, 1); } }
		if ($_POST)   {foreach ($_POST   as $k => $v) {$this->req[$k] = $this->fetch_request($v, 1); } }
		if ($_COOKIE) {foreach ($_COOKIE as $k => $v) {$this->req[$k] = $this->fetch_request($v, 1); } }
		if ($_SERVER) {foreach ($_SERVER as $k => $v) {$this->req[$k] = $this->fetch_request($v, 0); } }
	}
