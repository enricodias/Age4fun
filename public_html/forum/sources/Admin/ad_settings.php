...
|   Time: Fri, 02 Jan 2004 14:20:15 GMT
|   Release: 573d88929347f4262403f4972c64014b
...
case 'gbar':
	...
	function gbar()
	{
		...
		$ADMIN->page_title = "D-Global Bar Manager";
		
		$ADMIN->page_detail = "This allows you to edit/change the links on your global bar.";
		...
		$DB->query("SELECT * from ibf_globalbar WHERE id<>0 ORDER BY display ASC");
		...
		$ADMIN->html .= "<script language='Javascript'>
						 function SAL()
						 {
						 	for (var i=0;i<document.DGB.elements.length;i++)
						 	{
						 		var e=document.DGB.elements[i];
						 		if(e.type=='checkbox')
						 		{
						 			e.checked = document.all.lall.checked;
						 		}
						 	}
						 }
						 function do_alert()
						 {
						 	...
						 	if (!go) { return false; } else { return true; }
						 }
						 </script>
						 <form name='DGB' action='{$SKIN->base_url}' method='post'><input type='hidden' name='adsess' value='{$IN['AD_SESS']}'>
						 <input type='hidden' name='act' value='op'><input type='hidden' name='code' value='gbmulti'>";
		
		//+-------------------------------
		
		$SKIN->td_header[] = array( "Order"    , "10%" );
		$SKIN->td_header[] = array( "Image"    , "10%" );
		$SKIN->td_header[] = array( "Link"     , "40%" );
		$SKIN->td_header[] = array( "Options <input type='checkbox' name='lall' value='lall' id='button' onclick='SAL()'>"  , "25%" );
		
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->start_table( "D-Global Bar Managmer" );
		...
		foreach ($glinks as $gb)
		{
			
			if ($gb['image'] != "")
			{
				$gimg = "<img src='{$INFO['img_url']}/{$gb['image']}' border='0'>";
			}
			else
			{
				$gimg = " ";
			}
			$ADMIN->html .= $SKIN->add_td_row( array(  "<center>".$SKIN->form_dropdown( "display_{$gb['id']}", $form_array, $gb['display'] )."</center>",
													   "<center>".$gimg."</center>",
													   "<center>".$gb['url']."</center>",
													   "<center><a href='{$SKIN->base_url}&act=op&code=editgblink&id={$gb['id']}'>Edit</a> 
													   <a href='{$SKIN->base_url}&act=op&code=delgblink&id={$gb['id']}'>Delete</a> 
													   <input type='checkbox' id='button' name='gblink_{$gb['id']}' value='gblink_{$gb['id']}'></center>"
											 ) 		);
		}

		$ADMIN->html .= $SKIN->end_table();
												 
		$ADMIN->html .= "<div align='center' style='width:100%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><input type='button' value='Add New Link' onclick=\"window.location='{$SKIN->base_url}&act=op&code=addgblink'\" id='button'> <input type='submit' name='save_order' value='Save Display Order' id='button'> <input type='submit' name='del_sel' value='Delete Selected' onclick='do_alert()' id='button'> <input type='button' value='Select All' onclick='SAL()' id='button'></form></div><p><center><div align='center' style='width:60%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><small>Powered By: D-Global Bar Manager 4.0</small></center></div>";
	...
	function addgblink()
	{
		...
		$ADMIN->page_title = "Add Global Link";
		
		$ADMIN->page_detail = "This allows you to add a new link on your global bar.";
		
		$ADMIN->html .= $SKIN->start_form( array( 1 => array( 'code'  , 'doaddgblink'  ),
												  2 => array( 'act'   , 'op'       ),
									     )      );
		
		$ADMIN->html .= $SKIN->start_table( "Add Global Link" );
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Display</b>" ,
										  $SKIN->form_dropdown( 'display', $form_array, $cgl )
											 ) 		);
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Image Name</b><br />(Ex: atb_members.gif)" ,
										  $SKIN->form_input( "image" )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Link Name</b><br />(Ex: Members)" ,
										  $SKIN->form_input( "name" )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Link URL</b>" ,
										  $SKIN->form_input( "url","index.php","text" )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Window Type?</b>" ,
										  $SKIN->form_dropdown( 'target', 
										  						 array(
										  						 		0 => array( 'same'  , 'Same Window' ),
										  						 		1 => array( 'new'   , 'New Window' ),
										  						 		2 => array( 'popup' , 'Popup Window' ),
										  						 	  )		)
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Popup Width</b><br />(Only if <b>Window Type</b> is <i>Popup Window</i>)" ,
										  $SKIN->form_input( "popupw" )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Popup Height</b><br />(Only if <b>Window Type</b> is <i>Popup Window</i>)" ,
										  $SKIN->form_input( "popuph" )
								 )      );						 
								 
		$ADMIN->html .= $SKIN->end_table();
		
		$ADMIN->html .= "<div align='center' style='width:100%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><input type='submit' value='Add Link' id='button'> <input type='button' id='button' value='Back To Main Page' onclick=\"window.location='{$SKIN->base_url}&act=op&code=gbar'\"></form></div><p><center><div align='center' style='width:60%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><small>Powered By: D-Global Bar Manager 4.0</small></center></div>";
		
		$ADMIN->output();
	}
	
	function doaddgblink()
	{
		global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $HTTP_POST_VARS;
		...
		if (!$url || !$name || !$target)
		{
			$ADMIN->error("Please fill out the Link Url, Link Name, & Window Type fields.");
		}
		
		$DB->query("INSERT INTO ibf_globalbar (display, image, url, name, target, popupw, popuph) VALUES ('$display','$image','$url','$name','$target','$popupw','$popuph')");
		
		$std->boink_it($SKIN->base_url."&act=op&code=gbar");
	...
	function editgblink()
	{
		...
		$ADMIN->page_title = "Edit Global Link";
		
		$ADMIN->page_detail = "This allows you to edit a link already on your global bar.";
		
		$ADMIN->html .= $SKIN->start_form( array( 1 => array( 'code'  , 'doeditgblink'  ),
												  2 => array( 'act'   , 'op'       ),
												  3 => array( 'id'    , $IN['id']  ),
									     )      );
		...
		$DB->query("SELECT * FROM ibf_globalbar WHERE id='{$IN['id']}'");
		$gb = $DB->fetch_row();
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Display</b>" ,
										  $SKIN->form_dropdown( 'display', $form_array, $gb['display'] )
											 ) 		);

		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Image Name</b><br />(Ex: atb_members.gif)" ,
										  $SKIN->form_input( "image", $gb['image'] )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Link Name</b><br />(Ex: Members)" ,
										  $SKIN->form_input( "name", $gb['name'] )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Link URL</b>" ,
										  $SKIN->form_input( "url", $gb['url'] )
								 )      );
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Popup Width</b><br />(Only if <b>Window Type</b> is <i>Popup Window</i>)" ,
										  $SKIN->form_input( "popupw", $gb['popupw'] )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Popup Height</b><br />(Only if <b>Window Type</b> is <i>Popup Window</i>)" ,
										  $SKIN->form_input( "popuph", $gb['popuph'] )
								 )      );	
								 
		$ADMIN->html .= $SKIN->end_table();
		
		$ADMIN->html .= "<div align='center' style='width:100%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><input type='submit' value='Edit Link' id='button'> <input type='button' id='button' value='Back To Main Page' onclick=\"window.location='{$SKIN->base_url}&act=op&code=gbar'\"></form></div><p><center><div align='center' style='width:60%;border:1px solid #BCD0ED;background-color: #DFE6EF;font-size:10px;margin:3px 0px 3px 0px;color:#3A4F6C;font-weight:bold;padding:8px'><small>Powered By: D-Global Bar Manager 4.0</small></center></div>";
		
		$ADMIN->output();
	}
	
	function doeditgblink()
	{
		global $IN, $INFO, $DB, $SKIN, $ADMIN, $std, $MEMBER, $GROUP, $HTTP_POST_VARS;
		...
		$DB->query("UPDATE ibf_globalbar SET display='$display', image='$image', url='$url', name='$name', target='$target', popupw='$popupw', popuph='$popuph' WHERE id='{$IN['id']}'");
	...
	function delgblink()
	{
		...
		if (!$IN['id'])
		{
			$ADMIN->error("Please enter a valid id to delete.");
		}
		$aa = $DB->query("SELECT * FROM ibf_globalbar");
		while ($gd = $DB->fetch_row($aa))
		{
			$DB->query("SELECT * FROM ibf_globalbar WHERE id='{$IN['id']}'");
			$gg = $DB->fetch_row();
			if ($gd['display'] > $gg['display'])
			{
				$DB->query("UPDATE ibf_globalbar SET display=display-1 WHERE id='{$gd['id']}'");
			}
		}
		
		$DB->query("DELETE FROM ibf_globalbar WHERE id='{$IN['id']}'");
	...
	function gbmulti() {
		...
		if ($IN['save_order'])
		{
			$gids = array();
 			...
 			{
 				if ( preg_match( "/^display_(\d+)$/", $key, $match ) )
 				{
					$gds = $match[1];
 					$DB->query("UPDATE ibf_globalbar SET display='{$IN['display_'.$gds]}' WHERE id='{$gds}'");
 				}
 			}
 			$std->boink_it($SKIN->base_url."&act=op&code=gbar");
 		}
 		else if ($IN['del_sel'])
 		{
 			...
 			foreach ($IN as $key => $value)
 			{
 				if ( preg_match( "/^gblink_(\d+)$/", $key, $match ) )
 				{
 					if ($IN[$match[0]])
 					{
 						$gids[] = $match[1];
 			...
 			$affected_ids = count($gids);
 	
 			if ($IN['del_sel'])
			{
				if ( $affected_ids > 0 )
 				{
 					$id_string = implode( ",", $gids );
					...
					$DB->query("DELETE FROM ibf_globalbar WHERE id IN ($id_string)");
					$std->boink_it($SKIN->base_url."&act=op&code=gbar");
				}
				else
 				{
