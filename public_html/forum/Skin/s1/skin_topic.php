...
return <<<EOF
    ...
    <tr>
      <td valign='middle' class='row4' width="1%"><a name='entry{$post['pid']}'></a><span class='{$post['name_css']}'>{$author['name']}</span></td>
        ...
        {$post['post_icon']}<span class='postdetails'><b><b><a title="'.$ibforums->lang['tt_link'].'" href="#" onclick="link_to_post(); return false;" style="text-decoration:underline">'.$ibforums->lang['posted_on'].'</a></b> {$post['post_date']}</span>
        </div>
