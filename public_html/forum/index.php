...
//--------------------------------
//  D-Global Bar Manager
//--------------------------------
$DB->query("SELECT * FROM ibf_globalbar");
$gln = $DB->get_num_rows();
$glnum = round( $gln/2 );
$glb = $DB->query("SELECT * FROM ibf_globalbar ORDER BY display ASC");
while ( $gb = $DB->fetch_row($glb) )
{
	if ($gb['target'] == 'new')
	{
		if ($gb['image'])
		{
			$glinks .= "<img src='{$ibforums->vars['img_url']}/{$gb[image]}' border='0'> ";
		}
		$glinks .= "<a href='{$gb['url']}' target='_blank'>{$gb['name']}</a>";
		if ($gb['display'] == $glnum)
		{
			$glinks .= "<br>";
		}
	...
	else if ($gb['target'] == 'popup')
	{
		if ($gb['image'])
		{
			$glinks .= "<img src='{$ibforums->vars['img_url']}/{$gb[image]}' border='0'> ";
		}
		$glinks .= "<a href=# onclick=\"window.open('{$gb['url']}','','width={$gb['popupw']},height={$gb['popuph']},scrollbars=yes,toolbar=yes,menubar=no,history=yes,resizable=no,left=0,top=0'); return false\">{$gb['name']}</a>";
		if ($gb['display'] == $glnum)
		{
			$glinks .= "<br>";
		}
	...
	else if ($gb['target'] == 'same')
	{
		if ($gb['image'])
		{
			$glinks .= "<img src='{$ibforums->vars['img_url']}/{$gb[image]}' border='0'> ";
		}		
		$glinks .= "<a href='{$gb['url']}'>{$gb['name']}</a>";
		if ($gb['display'] == $glnum)
		{
			$glinks .= "<br>";
		}
